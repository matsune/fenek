...
fn test_parse_expr() {
    ...
    let ident = parser.parse_expr().unwrap().as_ident();
...
fn test_parse_expr1() {
    ...
    let binary = parser.parse_expr().unwrap().as_binary();
    assert_eq!(binary.op, BinOp::Add);
    assert_eq!(binary.lhs.as_lit().raw, "1");
    let rhs = binary.rhs.as_binary();
    assert_eq!(rhs.op, BinOp::Mul);
    assert_eq!(rhs.lhs.as_ident().raw, "a");
    assert_eq!(rhs.rhs.as_lit().raw, "2");
}
...
fn test_parse_expr2() {
    ...
    assert_eq!(lhs.lhs.as_lit().raw, "1");
    assert_eq!(lhs.rhs.as_ident().raw, "a");
    assert_eq!(binary.rhs.as_lit().raw, "2");
}
...
fn test_parse_expr3() {
    ...
    let binary = parser.parse_expr().unwrap().as_binary();
    assert_eq!(binary.op, BinOp::Sub);
    assert_eq!(binary.lhs.as_ident().raw, "b");
    let rhs_binary = binary.rhs.as_binary();
    assert_eq!(rhs_binary.op, BinOp::Mul);
    assert_eq!(rhs_binary.lhs.as_lit().raw, "3");
    let rhs_lhs_binary = rhs_binary.rhs.as_binary();
    assert_eq!(rhs_lhs_binary.op, BinOp::Div);
    assert_eq!(rhs_lhs_binary.lhs.as_ident().raw, "a");
    assert_eq!(rhs_lhs_binary.rhs.as_lit().raw, "2");
}
...
fn test_parse_expr4() {
    ...
    assert_eq!(lhs_binary.rhs.as_lit().raw, "1");
    let lhs_lhs_binary = lhs_binary.lhs.as_binary();
    assert_eq!(lhs_lhs_binary.op, BinOp::Sub);
    assert_eq!(lhs_lhs_binary.lhs.as_lit().raw, "4");
    assert_eq!(lhs_lhs_binary.rhs.as_lit().raw, "3");
}

#[test]
fn test_parse_expr5() {
    let mut parser = make_parser!("!(4 - 3)");
    let unary = parser.parse_expr().unwrap().as_unary();
    assert_eq!(unary.op, UnaryOp::Not);
    let binary = unary.expr.as_binary();
    assert_eq!(binary.op, BinOp::Sub);
    assert_eq!(binary.lhs.as_lit().raw, "4");
    assert_eq!(binary.rhs.as_lit().raw, "3");
}
...
fn test_parse_var_decl() {
    ...
    assert_eq!(var_decl.init.as_lit().raw, "3");
}
