...
impl<'ctx> Codegen<'ctx> {
    ...
    fn build_stmt(&mut self, function: &RefCell<Function<'ctx>>, stmt: &hir::Stmt) {
        match stmt {
            hir::Stmt::VarDecl(var_decl) => {
                ...
                let val = self.build_expr(&function.borrow(), &var_decl.init).unwrap();
                let ptr = function
                    .borrow()
                    .builder
            ...
            hir::Stmt::Ret(ret) => match &ret.expr {
                Some(expr) => {
                    let val = self.build_expr(&function.borrow(), &expr).unwrap();
            ...
            hir::Stmt::Assign(assign) => {
                let left = self.build_expr_left(&function.borrow(), &assign.left);
                let right = self.build_expr(&function.borrow(), &assign.right).unwrap();
                function
                    .borrow()
                    .builder
    ...
    fn build_expr_left(
        ...
        match expr {
            hir::Expr::Path(ident) => function.var_map.get(&ident.def.id).unwrap().ptr.into(),
            hir::Expr::DerefExpr(deref_expr) => {
                let ptr = self
                    .build_expr_left(function, &deref_expr.expr)
                    .into_pointer_value();
                function.builder.build_load(ptr, "")
            }
            hir::Expr::RefExpr(ref_expr) => {
                let expr = self.build_expr_left(function, &ref_expr.expr);
                let ptr = function.builder.build_alloca(expr.get_type(), "");
                function.builder.build_store(ptr, expr);
                ptr.into()
            }
            _ => unreachable!(),
        }
    ...
    fn build_expr(
        ...
        let v = match expr {
            ...
            hir::Expr::Path(ident) => {
                let ptr = function.var_map.get(&ident.def.id).unwrap().ptr;
                function.builder.build_load(ptr, &ident.raw)
            }
            hir::Expr::Call(call) => {
                let mut args = Vec::new();
                for arg in &call.args {
                    args.push(self.build_expr(&function, &arg).unwrap());
                }
            ...
            hir::Expr::Binary(binary) => {
                let lhs = self.build_expr(function, &binary.lhs).unwrap();
                let rhs = self.build_expr(function, &binary.rhs).unwrap();
                match binary.ty {
                    ty::Type::Int(_) => {
            ...
            hir::Expr::NegExpr(neg_expr) => {
                let expr = self.build_expr(function, &neg_expr.expr).unwrap();
            ...
            hir::Expr::NotExpr(not_expr) => {
                let expr = self.build_expr(function, &not_expr.expr).unwrap();
            ...
            hir::Expr::RefExpr(ref_expr) => self.build_expr_left(function, &ref_expr.expr),
            hir::Expr::DerefExpr(ref_expr) => {
                let value = self.build_expr(function, &ref_expr.expr).unwrap();
